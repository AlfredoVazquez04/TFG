...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
        ...
        # --- MODIFICACIÓN DINÁMICA: Ajustamos el stem_layer ---
        # Extraemos el img_size real de los kwargs si existe, si no, usamos el por defecto de synapse
        img_size = kwargs.get('img_size', (64, 128, 128))
        
        # Calculamos el stride dinámico
        stem_stride = (
            img_size[0] // 32, 
            img_size[1] // 32, 
            img_size[2] // 32
        ...
        stem_layer = nn.Sequential(
            get_conv_layer(spatial_dims, in_channels, dims[0], kernel_size=stem_stride, stride=stem_stride,
