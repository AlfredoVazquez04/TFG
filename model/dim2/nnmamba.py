...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    return nn.Conv2d( # CORREGIDO a Conv2d
        in_planes,
        out_planes,
        kernel_size=3,
        stride=stride,
...
def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution."""
    return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False) # CORREGIDO a Conv2d
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, mamba_layer=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes) # CORREGIDO a BatchNorm2d
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = nn.BatchNorm2d(planes) # CORREGIDO a BatchNorm2d
...
def make_res_layer(inplanes, planes, blocks, stride=1, mamba_layer=None):
    downsample = nn.Sequential(
        ...
        nn.BatchNorm2d(planes), # CORREGIDO a BatchNorm2d
    )
...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2):
        ...
        self.norm2 = nn.BatchNorm2d(dim) # CORREGIDO a BatchNorm2d
        self.relu2 = nn.ReLU(inplace=True)
        self.relu3 = nn.ReLU(inplace=True)

        self.norm = nn.BatchNorm2d(dim) # CORREGIDO a BatchNorm2d
        self.relu = nn.ReLU(inplace=True)
        self.mamba = Mamba(
            d_model=dim, 
            d_state=d_state,  
...
class DoubleConv(nn.Module):
    def __init__(self, in_ch, out_ch, stride=1, kernel_size=3):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=kernel_size, stride=stride, padding=int(kernel_size / 2)), # CORREGIDO
            nn.BatchNorm2d(out_ch), # CORREGIDO
            nn.ReLU(inplace=True),
            nn.Conv2d(out_ch, out_ch, 3, padding=1, dilation=1), # CORREGIDO
            nn.BatchNorm2d(out_ch), # CORREGIDO
            nn.ReLU(inplace=True),
        )
...
class SingleConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_ch, out_ch, 3, padding=1), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True)) # CORREGIDO
...
class nnMambaSeg(nn.Module):
    def __init__(self, in_ch=1, channels=32, blocks=3, number_classes=6):
        ...
        self.pooling = nn.AdaptiveAvgPool2d((1, 1)) # CORREGIDO a 2D
        ...
        # CORREGIDO mode='trilinear' -> 'bilinear'
    ...
    def forward(self, x):
        x = x.contiguous()
        c1 = self.in_conv(x)
        # CORREGIDO reshape de 5D a 4D (1, 1) en lugar de (1, 1, 1)
