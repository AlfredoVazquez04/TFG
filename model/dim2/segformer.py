...
class MixVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.tf_block1:
            x = blk(x)
        x = self.norm1(x)
        x = x.reshape(B, n, n, -1).permute(0, 3, 1, 2).contiguous() # Corregido: se reasigna a 'x'
        out.append(x)
        ...
        for blk in self.tf_block3:
            x = blk(x)
        x = self.norm3(x)
        x = x.reshape(B, n, n, -1).permute(0, 3, 1, 2).contiguous() # Corregido a 2D
        out.append(x)
...
class DWConv(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        n = square_root(N)
        x = x.transpose(1, 2).view(B, C, n, n) # Corregido a 4 dimensiones (2D)
        x = self.dwconv(x)
        x = self.bn(x)
...
class SegFormerDecoderHead(nn.Module):
    ...
    def forward(self, c1, c2, c3, c4):
        # Desempaquetar 4 dimensiones (Batch, Channel, Height, Width)
        n, _, _, _ = c4.shape 

        _c4 = (
            self.linear_c4(c4)
            .permute(0, 2, 1)
            .reshape(n, -1, c4.shape[2], c4.shape[3]) # Corregido sin c4.shape[4]
            .contiguous()
        )
        ...
        _c3 = (
            self.linear_c3(c3)
            .permute(0, 2, 1)
            .reshape(n, -1, c3.shape[2], c3.shape[3]) # Corregido sin c3.shape[4]
            .contiguous()
        )
        ...
        _c2 = (
            self.linear_c2(c2)
            .permute(0, 2, 1)
            .reshape(n, -1, c2.shape[2], c2.shape[3]) # Corregido sin c2.shape[4]
            .contiguous()
        )
        ...
        _c1 = (
            self.linear_c1(c1)
            .permute(0, 2, 1)
            .reshape(n, -1, c1.shape[2], c1.shape[3]) # Corregido sin c1.shape[4]
            .contiguous()
        )
