...
class AMOSDataset(pl.LightningDataModule):
    """Class to define the dataset AMOS22 for 2D (.npy) images.
    """   
    organs = [
    ...
    def get_preprocessing_transform_pred(self):
        ...
        val_test_transforms_0 = Compose(
            [
                EnsureChannelFirstd(keys=self.keys[0], channel_dim="no_channel"),
                ScaleIntensityRanged(
                    keys=self.keys[0],
                    a_min=self.scale_range[0],
                    a_max=self.scale_range[1],
    ...
    def __get_data(self, folders_img_lbl=True):
        """Function to get the data from the path (now reading .npy).
        ...
        """        
        if folders_img_lbl:
            data_images = sorted(glob.glob(os.path.join(self.args.data_dir, "imagesTr", "*.npy")))
            data_labels = sorted(glob.glob(os.path.join(self.args.data_dir, "labelsTr", "*.npy")))
    ...
    def __get_data_pred(self, folders_img_lbl=True):
        """Function to get the data from the path for prediction (now reading .npy).
